    ...
    for k in rxg_consts.Node_URLs.__dict__:
        if k.find('__') < 0:
            if k not in ('EXPORT_FMT', ):
                setattr(rxg_consts.Node_URLs, k, os_path.join(g_vars.g_cfg['app_url_prefix'], rxg_consts.Node_URLs.__dict__[k]))

    for k in rxg_consts.Keys.__dict__:
        if k.find('__') < 0:
            temp = os_path.join(g_vars.g_cfg['app_url_prefix'], rxg_consts.Keys.__dict__[k])
            setattr(rxg_consts.Keys, k, temp.replace('/', '_'))
